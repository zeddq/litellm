...
async def proxy(request: Request, path: str):
    ...
    try:
        ...
        headers = {
            ...
            if k.lower() not in [ch.lower() for ch in CUSTOM_HEADERS]
        }
        ...
        if request_body:
            try:
                ...
                if 'thinking' in body_data:
                    ...
                    anthropic_thinking["budget_tokens"]= round_thinking(anthropic_thinking["budget_tokens"])
        ...
        if is_stream_request:
            logger.info(f"{rid} Handling as streaming response")
                ...
                async with httpx.AsyncClient(timeout=300.0) as client:
                    try:
                        async with client.stream(
                            ...
                            logger.info(f"{rid} Stream status: {resp.status_code}")

                            # Stream chunks as they arrive
                            async for chunk in resp.aiter_raw():
                                if chunk:
                                    logger.debug(f"{rid} Chunk: {len(chunk)} bytes")
                                    yield chunk
