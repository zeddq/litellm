    ...
    def log_success_event(self, kwargs: Dict[str, Any], response_obj: Any, start_time: float, end_time: float):
        ...
        try:
            ...
            if last_message.get("role") == "tool":
                ...
                logger.log(log_level, f"🔍 [ToolDebug] Round Analysis (ToolCallID: {tool_call_id})")
                
                # 1. What we sent (The feedback/result)
                logger.log(log_level, f"   ➡️ Sent Context (Last Message):\n{tool_content[:500]}...")
                ...
                if response_obj and hasattr(response_obj, "choices") and response_obj.choices:
                    ...
                    if hasattr(reaction, "tool_calls") and reaction.tool_calls:
                        for tc in reaction.tool_calls:
                            logger.log(log_level, f"   ⬅️ LLM Reaction (New Tool Call): {tc.function.name}({tc.function.arguments})")
                    else:
                        # Or did it give up/reply with text?
                        content = reaction.content or ""
                        logger.log(log_level, f"   ⬅️ LLM Reaction (Text): {content[:200]}...")
